...
export default function Footer({ logoUrl }: FooterProps) {
  ...
  return (
    <footer className="bg-[#4BB8FA] text-white">
      <div className="container-content py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
        {/* Brand */}
        <div className="lg:col-span-1">
          <Link href="/" className="flex items-center gap-2 text-white mb-4" aria-label="Home">
        ...
        {/* Quick links */}
        <div>
          <h3 className="font-semibold text-sm uppercase tracking-widest text-white mb-4">
            Navigate
          </h3>
          <ul className="space-y-2">
            {QUICK_LINKS.map(({ href, label }) => (
              <li key={href}>
                <Link href={href} className="text-white/85 hover:text-white text-sm transition-colors">
                  {label}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm text-white/85">
            <li className="flex items-start gap-2">
              <MapPin size={15} className="shrink-0 mt-0.5 text-white" aria-hidden="true" />
              <span>{LOCATION_ADDRESS}</span>
            </li>
            <li className="flex items-center gap-2">
              <Phone size={15} className="shrink-0 text-white" aria-hidden="true" />
              <a href={`tel:${WHATSAPP_NUMBER}`} className="hover:text-white transition-colors">
                {WHATSAPP_NUMBER}
              </a>
            </li>
            <li className="flex items-center gap-2">
              <Mail size={15} className="shrink-0 text-white" aria-hidden="true" />
              <a href={`mailto:${BUSINESS_EMAIL}`} className="hover:text-white transition-colors">
                {BUSINESS_EMAIL}
              </a>
            </li>
            <li className="flex items-center gap-2">
              <Clock size={15} className="shrink-0 text-white" aria-hidden="true" />
          ...
          <div className="flex gap-3 mt-5">
            <a href={SOCIAL.instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="w-9 h-9 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center transition-colors">
              <Instagram size={16} aria-hidden="true" />
            </a>
            <a href={SOCIAL.facebook} target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="w-9 h-9 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center transition-colors">
              <Facebook size={16} aria-hidden="true" />
            </a>
            <a href={SOCIAL.tiktok} target="_blank" rel="noopener noreferrer" aria-label="TikTok" className="w-9 h-9 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center transition-colors">
      ...
      <div className="border-t border-white/10">
        <div className="container-content py-4 flex flex-col sm:flex-row justify-between items-center gap-2 text-xs text-white/80">
